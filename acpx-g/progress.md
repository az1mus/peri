...
## R49 — Design Review Round 27 (用户思维)

`editorSave`/`editorRun` 验证失败时 toast 直接显示前 3 条具体错误（此前仅提示"修复验证错误"，用户需手动查找验证徽章才能看到错误内容）；`api` 函数捕获网络异常（`fetch` 抛出的 TypeError）并显示中文提示"无法连接到服务器，请检查网络或服务是否运行"替代原始 "Failed to fetch"；新增 `test_parse_workflow_partial_defaults` 测试验证只指定 `retry` 时 `timeout`/`shell` 正确回退为默认值（160 → 161）。
