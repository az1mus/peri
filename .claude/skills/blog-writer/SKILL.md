...
- 先给数字锚定现实
- 技术术语第一次出现要带一句白话解释，不能默认读者都懂。比如「chunk（网络传输的一个数据块）」「max_tokens（模型一次最多能输出的字数上限）」
- 紧跟着说这个数据的实际意义
...
- **故事性叙事替代工程描述**：把技术分析写成情节，三类都要改。一，拟人化模型行为（模型「老老实实写完」「没遵守 schema」「撒谎」）→ 用中性动词描述实际输出（「字段排在前面」「stop_reason 与内容不一致」），模型是程序的输出方不是角色。二，戏剧化口语替代具体后果（「翻车」「撑爆」「堵死」「能忍」）→ 写出实际影响（「中断长任务」「超出 token 上限」「消除该类错误」）。三，叙事过渡引子（「先说结论」「这就是为什么 X」「修复的关键认识是」「下面按顺序讲」）→ 删掉，直接进入机制。语气可以直、可以批判，但不把模型当角色演
