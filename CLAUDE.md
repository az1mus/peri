...
Agent 工具支持 `run_in_background: true`，子 agent 在后台 `tokio::spawn` 中运行，父 agent 立即继续。

- **通道架构**：后台 agent 不共享父的 `event_handler`（避免 TextChunk/ToolStart 等子 agent 事件混入父消息流）。完成通知通过 `BackgroundTaskCompleted` 事件单独发送。
- **双通道通知**：`BackgroundTaskRegistry.complete()` 通过 `notification_tx`（UnboundedChannel）通知 executor 注入 state；同时 spawn closure 通过 `event_handler.on_event()` 通知 TUI 更新 UI。两个通道独立。
- **通道生命周期**：后台 spawn closure 持有 `event_handler` 的 Arc clone。当 closure 结束（后台任务完成 + 事件发送后），Arc drop → `tx_event` drop → 可能触发通道关闭。`poll_agent()` 的 `Disconnected` 处理需检测 `agent_done_pending_bg` / `background_task_count` 抑制误报。
- **并发限制**：`BackgroundTaskRegistry` 最大 3 个并发后台任务。
- **必需参数**：后台模式必须提供 `subagent_type`（无 fork 支持）。
- **自动 continuation**：父 agent Done 后若有后台任务，`agent_done_pending_bg = true` 保持通道存活；最后一个后台任务完成时触发 `pending_bg_continuation` 机制自动提交 continuation 消息。
- **agent_id 格式**：`format_available_agents()` 输出格式为 `- {agent_id}: {description}`，以 agent_id（文件名去 .md）作为主标识符，避免 LLM 混淆 frontmatter `name` 字段与实际 ID。
...
| `LlmRetrying` | LLM 重试中 |
| `BackgroundTaskCompleted` | 后台 agent 任务完成（TUI 使用） |

TUI 层扩展：`Done` / `Error` / `ApprovalNeeded` / `AskUserBatch` / `SubAgentStart` / `SubAgentEnd` / `BackgroundTaskCompleted`（含详细字段）。
